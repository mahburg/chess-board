  ...
  selectSquare(i){
    ...
    if(this.state.possible.includes(i)){
      if(this.state.board.hasOwnProperty(i)){
        ...
        if (color === 'w'){
          let arr = this.state.elimW.slice();
          arr.push(this.state.board[i]);
          this.setState({
            elimW: arr
          })
        } else{
          let arr = this.state.elimB.slice();
          arr.push(this.state.board[i]);
          this.setState({
            elimB: arr
          })
...
// eslint-disable-next-line
      switch (piece) {
        case 'p':
            ...
            if ((color === 'b' && coord.r === 2 )||(color === 'w' && coord.r === 7 )){
  ...
  render() {
    ...
    const squares = arr.map((c,i)=>{
      ...
      return <ChessSquare key={i} i={i} color={color} piece={piece} selected={i===state.selected} green={state.possible.includes(i)} selectMe={this.selectSquare}/>
    }
    )
    const whiteElim = state.elimW.map((c,i)=>{
      const color = c[0];
      const piece = c.slice(1);
      return <PieceSquare color={color} piece={piece} i={i} key={i} />
    })
    const blackElim = state.elimB.map((c,i)=>{
      const color = c[0];
      const piece = c.slice(1);
      return <PieceSquare color={color} piece={piece} i={i} key={i} />
    })
    return (
      <div className="App">
        <div className="App-header">
          <h2 className='App-intro' onClick={_=>this.resetBoard()}>Chess</h2>
          <img src={logo} className="App-logo" alt="logo" />
        </div>
        <div className="content">
          <div className="black graveyard">
            {blackElim}
          </div>
          <div className="board">
              {squares}
          </div>
          <div className="white graveyard">
            {whiteElim}
          </div>
